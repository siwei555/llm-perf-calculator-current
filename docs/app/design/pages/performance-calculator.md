...
`公式追溯` 必须位于性能对比图下方，并按 `Prefill`、`Decode`、`Memory` 小标题形成三行独立折叠区。同一阶段的平台缩放公式与模型算子公式合并在一个折叠区内，不得生成重复的 `Prefill` 或 `Decode` 分块。三个折叠区首次进入均为收起状态，可分别展开；展开后该阶段的公式项按一行三列排列并顶部对齐。

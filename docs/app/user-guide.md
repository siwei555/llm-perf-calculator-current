...
- 可供程序处理的 JSON 计算快照；
- 保存在本机的计算历史。
- 最多四条独立配置的 Prefill、Decode 和显存看板对比。

## 多芯片与性能看板

平台算力、内存带宽和显存容量均按“单芯片值”填写。`N Chip` 会线性汇总三项平台能力；设为 `1` 时与原有单芯片计算完全一致。该估算暂不模拟跨芯片通信损耗，正式多卡规划应通过实测调整效率系数。

Precision Preset 只是字节参数快捷设置：

- `W4A8`：Weight 0.5、Expert 0.5、Activation 1 byte；
- `W8A8`：三项均为 1 byte；
- `FP8`：三项均为 1 byte，但保留 FP8 语义标签；
- `BF16`：三项均为 2 bytes。

使用对比看板：

1. 在当前输入区设置模型和平台参数，点击“添加当前配置”。
2. 可复制配置，并直接修改配置卡中的 Batch、N Chip 和 Precision。
3. 最多保存四条配置；取消勾选可暂时排除某条配置。
4. 点击“开始对比”，生成 Prefill Speed、Initial Decode Speed 和 Memory Usage 三张图及明细表。
5. 所有配置共享当前 Token Sweep；修改当前输入不会改动已经保存的配置。

Memory Usage 支持“全部”“只看 Weight”“只看 KV/Cache”。柱形 tooltip 会继续显示权重、持久缓存/状态、临时工作集、运行时开销和总量。

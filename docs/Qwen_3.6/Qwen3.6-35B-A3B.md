# Qwen3.6-35B-A3B 架构结构图（ASC）

> 状态：结构分析已重写，等待用户确认后进入工程适配。  
> 基于官方 [`config.json`](./config/Qwen3.6-35B-A3B-config.json) 与
...
> 图像 token 预处理和 MTP speculative decoding。
...
Base checkpoint 的文本 `dtype=bfloat16`，未声明 `quantization_config`。

- 推荐权重口径：2 B/param
- 推荐 activation/cache 口径：2 B/elem
- DeltaNet recurrent state 按 FP32（4 B/elem）估算
...
> `B=1`，初始上下文 `S_ctx=131072`。Base 权重和 KV 使用 BF16；
> recurrent state 使用 FP32。
...
```text
M_weights = 34.66B * 2 B = 69.32 GB
```

该值不含 allocator、框架元数据和通信 buffer。
...
M_decode_traffic ≈ 9.340 GB/token
```
...
- 官方配置：
  `docs/Qwen_3.6/config/Qwen3.6-35B-A3B-config.json`
- 官方模型页：
...
python scripts/analyze_qwen36_config.py `
  docs/qwen3.6/config/qwen3.6-35b-a3b-config.json `
  --expect-layers 40 `
